...
- The user requested all session work committed and merged. [PR #338](https://github.com/angellops/paypal-php-library/pull/338) integrates the documentation/context baseline into `main`. Check its live merge state and Git refs when resuming. The [branch policy](../agents/branching.md) keeps `main` v3-compatible before GA and uses `v4` for new implementation; create `v4` from that merged baseline if it does not yet exist. No release tag or package publication is part of this integration.
