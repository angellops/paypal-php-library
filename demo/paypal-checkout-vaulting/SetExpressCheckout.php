...
$SECFields = array(
	...
	'returnurl' => $domain . 'demo/paypal-checkout-vaulting/GetExpressCheckoutDetails.php', 							    // Required.  URL to which the customer will be returned after returning from PayPal.  2048 char max.
	'cancelurl' => $domain . 'demo/paypal-checkout-vaulting/', 							    // Required.  URL to which the customer will be returned if they cancel payment on PayPal's site.
...
/**
 * Now we add the billing agreement parameters so that we can
 * process reference transactions for this buyer in the future.
 */
...
$RESTVaultSetupData = array(
	'brand_name' => 'Angell EYE',
	'return_url' => $domain . 'demo/paypal-checkout-vaulting/GetExpressCheckoutDetails.php', 
	'cancel_url' => $domain . 'demo/paypal-checkout-vaulting/', 
);
...
if( $api_mode === 'classic' ) {
	...
	$PayPalResult = $PayPal->createVaultSetupToken($RESTVaultSetupData);
