...
if( $api_mode === 'classic' ) {
    ...
    $setupToken = isset($_SESSION['paypal_setup_token']) ? $_SESSION['paypal_setup_token'] : '';

    $getSetupTokenData = $PayPal->getVaultSetupTokenDetails($setupToken);

    if( $getSetupTokenData['success'] && $getSetupTokenData['status'] === 'APPROVED' ) {
        $approvedSetupToken = isset( $getSetupTokenData['setup_token'] ) ? $getSetupTokenData['setup_token'] : '';

        $VaultPaymentTokenData = [
            'payment_source' => [
                'token' => [
                    'id' => $approvedSetupToken,
                    'type' => 'SETUP_TOKEN'
                ],
            ]
        ];
    
        $RESTVaultPaymentTokenData = array(
            'setup_token' => $approvedSetupToken,
            'vault_payment_data' => $VaultPaymentTokenData,
        );

        $PayPalResult = $PayPal->createVaultPaymentToken($RESTVaultPaymentTokenData);

        if( $PayPalResult['success'] ) {
            $_SESSION['paypal_vault_token'] = isset($PayPalResult['vault_token']) ? $PayPalResult['vault_token'] : '';
