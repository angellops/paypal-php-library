      ...
      <div class="container">
        ...
        <div class="cart-intro">
          <p>We have now reached the final thank you / receipt page and the payment has been processed! We have added the PayPal Transaction ID to 
            the Billing Information, which was provided in the response.
          </p>
        </div>
        ...
        <div class="cart-items-card rv-items-card">
          <div class="cart-items-header">
            <h2>Order Items</h2>
            <?php if (!empty($_SESSION['shopping_cart']['checkout_options_items'])): ?>
              <span class="cart-count">
                <?php echo count($_SESSION['shopping_cart']['checkout_options_items']); ?> items
              </span>
            <?php endif; ?>
          </div>
        ...
        <div class="rv-bottom-grid">
          ...
          <div class="rv-info-card">
            ...
            <div class="rv-info-body">
              ...
              <?php } ?>
              <?php if( !empty( $_SESSION['card_last4'] ) || !empty( $_SESSION['card_brand'] ) ) { ?>
                <div class="rv-info-row">
                  <span class="rv-info-label">Card</span>
                  <?php $card_last = !empty($_SESSION['card_last4']) ? $_SESSION['card_last4'] : '';
                    $card_brand = !empty($_SESSION['card_brand']) ? $_SESSION['card_brand'] : '';
                    $card_name = trim($card_last . ' (' . $card_brand . ')'); ?>
                  <span class="rv-info-value">
                    <?php echo !empty($card_name) ? $card_name : ''; ?>
                  </span>
                </div>
              <?php } ?>
              <?php if ( ! empty( $_SESSION['payment_mode'] ) ) { 
                ...
                switch ($payment_mode) {
                  ...
                  case 'venmo':
                    ...
                    break;
                  case 'paylater':
                    $payment_label = 'PayPal - Pay Later';
                    break;
                  case 'guest':
                    $payment_label = 'Guest Checkout Payments';
                    break;
                  case 'acdc':
                    $payment_label = 'Advanced Credit/Debit Cards (ACDC)';
                    break;
                  default:
                    $payment_label = ucfirst($payment_mode);
                }
              ?>
                <div class="rv-info-row">
