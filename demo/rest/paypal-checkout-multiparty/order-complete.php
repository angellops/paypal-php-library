...
<html lang="en">
  <head>
    <meta charset="utf-8">
    <title>PayPal Checkout - Multiparty | Order Complete | PHP Class Library | Angell EYE</title>
    ...
    <main class="cart-main">
      <div class="container">
        ...
        <div class="cart-intro">
          <p>We have now reached the final thank you / receipt page and the payment has been processed!  We have added the PayPal transaction 
            IDs (one for each transaction in the Parallel Payment split) to the Billing Information, which was provided in the response.
          </p>
        </div>
        ...
        <div class="cart-items-card rv-items-card">
          <div class="cart-items-header">
            <h2>Order Items</h2>
            <?php if (!empty($_SESSION['shopping_cart']['multiparty_items'])): ?>
              <span class="cart-count">
                <?php echo count($_SESSION['shopping_cart']['multiparty_items']); ?> items
              </span>
            <?php endif; ?>
          </div>
          ...
          <div class="cart-table">
            <!-- Table Header -->
            <div class="cart-table-head cm-cart-table-head">
              <span class="col-id">ID</span>
              <span class="col-name">Name</span>
              <span class="col-fees">Fees</span>
            ...
            <!-- Item Rows -->
            <?php if (!empty($_SESSION['shopping_cart']['multiparty_items'])) {
              foreach ($_SESSION['shopping_cart']['multiparty_items'] as $cart_item) { ?>
                <div class="cart-table-row cm-cart-table-row">
                  <span class="col-id">
                  ...
                  <div class="col-fees">
                    <div class="item-fee item-ship">
                      <span class="item-fee-label">Shipping :</span>
                      <span class="item-fee-value"><?php echo '$' . number_format($cart_item['shipping'], 2); ?></span>
                    </div>
                    <div class="item-fee item-handling">
                      <span class="item-fee-label">Handling :</span>
                      <span class="item-fee-value"><?php echo '$' . number_format($cart_item['handling'], 2); ?></span>
                    </div>
                    <div class="item-fee item-tax">
                      <span class="item-fee-label">Tax :</span>
                      <span class="item-fee-value"><?php echo '$' . number_format($cart_item['tax'], 2); ?></span>
                    </div>
                  </div>
            ...
            <div class="rv-info-body">
              ...
              <div class="rv-info-row cp-txn-row">
                ...
                <span class="rv-info-value cp-txn-id">
                  <?php if (!empty($_SESSION['paypal_transaction_id'])) {
                    if (is_array($_SESSION['paypal_transaction_id'])) {
                      echo implode('<hr>', $_SESSION['paypal_transaction_id']);
                    } else {
                      echo $_SESSION['paypal_transaction_id'];
                    }
                  }
                  ?>
                </span>
